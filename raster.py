...
def rasterize_feature_data(feature_gdf: GeoDataFrame, grid: Grid2D) -> np.ndarray:

    # return an empty raster if there is no feature geometry
    if feature_gdf.empty:
        blank_raster = np.zeros(grid.shape, dtype=np.uint8)
        return blank_raster

    # Define the affine transform for the destination raster
    pixel_size = grid.hr
    transform = from_origin(
        grid.bounds["west"], grid.bounds["north"], pixel_size, pixel_size
    )

    # rasterize the feature polygons
    raster = rasterize(
